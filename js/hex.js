/* 2026 hero/opening rebuild feature flag */
window.hexExperienceV2=true;
...
const HOME_SECTIONS={
  ...
  OPENING:'gc_auto_frame_home_1', /* 開幕アニメーション */
  HERO:'gc_auto_frame_home_2', /* オリジナルヒーロー画像 */
  WELCOME:'gc_auto_frame_home_3', /* Welcomeメッセージ */
  ABOUT:'gc_auto_frame_home_4', /* 私たちについて */
  FIRST:'gc_auto_frame_home_5', /* 初めての方へ */
  SERVICE:'gc_auto_frame_home_6', /* サービス案内 */
  PICKUP:'gc_auto_frame_home_7', /* 注目アイテム */
  NEWS_SECTION:'gc_auto_frame_home_8', /* お知らせセクション */
  NEWS:'gc_auto_frame_home_9', /* 重要なお知らせ */
  BLOG:'gc_auto_frame_home_10', /* スタッフブログ */
  BANNER:'gc_auto_frame_home_11', /* バナー */
  MOVIE:'gc_auto_frame_home_12', /* プロモーション動画 */
  RECRUIT:'gc_auto_frame_home_13', /* 採用情報 */
  CONTACT:'gc_auto_frame_home_14', /* お問い合わせ */
  CALENDAR:'gc_auto_frame_home_15', /* 営業日カレンダー */
  AREA:'gc_auto_frame_home_16', /* 施工エリア */
  FOOTER:'gc_auto_frame_home_17', /* フッター */
  FIXED_FOOTER:'gc_auto_frame_home_18' /* 固定フッター */
};
...
function hexInitAnchorNav(){
  ...
  function animateFixedAnchorNav(){
    ...
    if(nav.hexUnfixTween){
      nav.hexUnfixTween.kill();
      nav.hexUnfixTween=null;
    }
  ...
  function animateAnchorNavUnfix(onComplete){
    var gsap=getAnchorGsap();
    ...
    nav.hexUnfixTween=gsap.timeline({
      onComplete:function(){
        nav.hexUnfixTween=null;
        onComplete();
      }
  ...
  function restoreFixedAnchorNav(){
    var gsap=getAnchorGsap();
  ...
  function updateHexAnchorNav(){
    ...
    if(scrollTop>=fixedStart){
      ...
      if(
        ...
        animateAnchorNavUnfix(function(){
          var currentScrollTop=
            window.pageYOffset||
            document.documentElement.scrollTop;

          /* フェード中に再び固定範囲へ戻った場合 */
          if(currentScrollTop>=fixedStart){
            restoreFixedAnchorNav();
            return;
          }
...
/* =======================================
   2026 rebuild: Three.js hero + WELCOME handoff
======================================= */
(function(){
  "use strict";
  ...
  function loadThree(){
    ...
    threePromise=new Promise(function(resolve,reject){
      var existing=document.querySelector("script[data-hex-three]");
      var script=existing||document.createElement("script");

      function complete(){
        if(window.THREE){resolve(window.THREE);}else{reject(new Error("THREE unavailable"));}
      }
  ...
  window.hexPreloadThree=loadThree;
  ...
  hexReady(function(){
    ...
    var welcomePanel=welcomeWrap&&(
      welcomeWrap.querySelector(".welcome.has-padding")||
      welcomeWrap.querySelector(".welcome")||
      welcomeWrap
    );
    var welcomeCopy=welcomeWrap&&welcomeWrap.querySelector(
    ...
    var welcomeStageExitTop=null;
    var welcomeDotPending=false;
    ...
    /* 診断版だけの表示計測。通常版の動作やスクロール計算は変更しない。 */
    var debugWelcomeSamples=[];
    var debugWelcomeHud=null;
    var debugWelcomeFixedTop=null;
    var debugWelcomeCircleLogged=false;
    var debugStageReleaseReference=null;
    var debugCircleReleaseReference=null;
    function debugWelcomeSnapshot(phase){
      if(!welcomeStage||!welcomePanel){return;}
      var stageRect=welcomeStage.getBoundingClientRect();
      var panelRect=welcomePanel.getBoundingClientRect();
      var imageRect=snapshot&&snapshot.getBoundingClientRect();
      var headRect=welcomeCopy&&welcomeCopy.getBoundingClientRect();
      var body=welcomeContents&&welcomeContents.querySelector('.text');
      var bodyRect=body&&body.getBoundingClientRect();
      var css=window.getComputedStyle(welcomeStage);
      var transformed=[];
      var parent=welcomeStage.parentElement;
      var sample;
      var round=function(value){return value==null?null:Math.round(value*100)/100;};
      while(parent&&parent!==document.documentElement){
        var style=window.getComputedStyle(parent);
        if(style.transform!=='none'||style.filter!=='none'||
          style.perspective!=='none'||style.contain!=='none'||
          style.willChange!=='auto'){
          transformed.push({
            element:parent.id||parent.className||parent.tagName,
            transform:style.transform,filter:style.filter,
            contain:style.contain,willChange:style.willChange
          });
        }
        parent=parent.parentElement;
      }
      if(css.position==='fixed'){
        debugWelcomeFixedTop=stageRect.top;
      }
      sample={
        phase:phase,time:Math.round(performance.now()),
        scrollY:round(window.pageYOffset),
        stage:{top:round(stageRect.top),bottom:round(stageRect.bottom),
          height:round(stageRect.height),cssPosition:css.position,
          inlineTop:welcomeStage.style.top,
          offsetParent:welcomeStage.offsetParent&&(
            welcomeStage.offsetParent.id||welcomeStage.offsetParent.className)},
        panelTop:round(panelRect.top),
        headingTop:headRect&&round(headRect.top),
        bodyTop:bodyRect&&round(bodyRect.top),
        circle:imageRect&&{top:round(imageRect.top),
          center:round(imageRect.top+imageRect.height*.5),
          height:round(imageRect.height),released:released},
        stageTopVariable:round(welcomeStageTop),
        screenTopVariable:round(welcomeStageScreenTop),
        exitTopVariable:round(welcomeStageExitTop),
        fixedTopReference:round(debugWelcomeFixedTop),
        differenceFromFixed:debugWelcomeFixedTop==null?null:
          round(stageRect.top-debugWelcomeFixedTop),
        ancestors:transformed
      };
      if(phase==='文字固定解除・直前'){
        debugStageReleaseReference={top:stageRect.top,scrollY:window.pageYOffset};
      }
      if(phase==='丸固定解除・直前'&&imageRect){
        debugCircleReleaseReference={top:imageRect.top,
          scrollY:window.pageYOffset};
      }
      if(debugStageReleaseReference&&phase.indexOf('文字固定解除・')===0){
        sample.stageTransitionShift=round(stageRect.top-
          debugStageReleaseReference.top+
          window.pageYOffset-debugStageReleaseReference.scrollY);
      }
      if(debugCircleReleaseReference&&imageRect&&
        phase.indexOf('丸固定解除・')===0){
        sample.circleTransitionShift=round(imageRect.top-
          debugCircleReleaseReference.top+
          window.pageYOffset-debugCircleReleaseReference.scrollY);
      }
      debugWelcomeSamples.push(sample);
      if(debugWelcomeSamples.length>90){debugWelcomeSamples.shift();}
      if(!debugWelcomeHud){
        debugWelcomeHud=document.createElement('div');
        debugWelcomeHud.style.cssText=
          'position:fixed;left:12px;bottom:12px;z-index:8500;'+
          'max-width:min(480px,90vw);padding:12px;background:#12213de8;'+
          'color:white;font:13px/1.55 monospace;white-space:pre-wrap;'+
          'box-shadow:0 4px 24px #0005;pointer-events:auto';
        var copyButton=document.createElement('button');
        copyButton.type='button';
        copyButton.textContent='WELCOME 診断ログをコピー';
        copyButton.style.cssText='display:block;margin:0 0 8px;padding:5px 10px;'+
          'font:13px sans-serif;cursor:pointer';
        copyButton.addEventListener('click',function(){
          var log=JSON.stringify(debugWelcomeSamples,null,2);
          if(navigator.clipboard&&navigator.clipboard.writeText){
            navigator.clipboard.writeText(log).then(function(){
              copyButton.textContent='コピーしました';
            }).catch(function(){window.prompt('ログをコピー',log);});
          }else{window.prompt('ログをコピー',log);}
        });
        debugWelcomeHud.appendChild(copyButton);
        var display=document.createElement('div');
        display.className='hex-welcome-debug-readout';
        debugWelcomeHud.appendChild(display);
        document.body.appendChild(debugWelcomeHud);
      }
      debugWelcomeHud.querySelector('.hex-welcome-debug-readout').textContent=
        phase+'\nscrollY '+sample.scrollY+' / '+css.position+
        '\n見出し '+sample.headingTop+'px  本文 '+sample.bodyTop+'px'+
        '\n全体 '+sample.stage.top+'px  文字切替差 '+
          sample.stageTransitionShift+'px'+
        '\n丸 '+(sample.circle?sample.circle.center:'なし')+'px'+
        '  丸切替差 '+sample.circleTransitionShift+'px'+
        '\n親 '+sample.panelTop+'px  top '+sample.stage.inlineTop;
      if(window.console&&console.debug){console.debug('[WELCOME診断]',sample);}
    }
    window.hexWelcomeDebug={samples:debugWelcomeSamples,
      dump:function(){return JSON.stringify(debugWelcomeSamples,null,2);}};
    ...
    function setHeroPageLock(locked){
      document.documentElement.classList.toggle(
        "hex-hero-manual-lock",
        !!locked
      );
    }
    ...
    function revealCatch(withFade){
      ...
      if(withFade){
        ...
        window.requestAnimationFrame(function(){
          window.requestAnimationFrame(function(){
            catchElement.classList.add("is-catch-visible");
          });
        });
      }else{
        catchElement.classList.add("is-catch-visible");
    ...
    function createWelcomeButton(){
      ...
      welcomeButton.className="hex-hero-welcome-button";
      welcomeButton.type="button";
      welcomeButton.setAttribute("aria-label","WELCOMEへ進む");
      welcomeButton.setAttribute("aria-hidden","true");
      welcomeButton.innerHTML=
        '<span>WELCOME</span><i aria-hidden="true"></i>';
      welcomeButton.addEventListener("click",function(event){
        event.preventDefault();
        if(!welcomeButton.classList.contains("is-visible")){return;}
        beginWelcome();
      });
    ...
    function createWelcomeStage(){
      if(!welcomePanel||welcomeStage){return;}

      welcomeStage=document.createElement("div");
      welcomeStage.className="hex-v2-welcome-copy-stage";

      if(welcomeCopy){
        welcomeCopy.classList.add(
          "hex-handoff-copy",
          "hex-welcome-copy",
          "hex-welcome-final-copy",
          "is-copy-active"
        );
        welcomeStage.appendChild(welcomeCopy);
      }

      if(welcomeContents&&welcomeContents!==welcomeCopy){
        welcomeStage.appendChild(welcomeContents);
      }
    ...
    function advanceWelcomeBody(now){
      var fraction=clamp((now-welcomeBodyStartedAt)/1000,0,1);
      welcomeBodyProgress=
        welcomeBodyStart+(welcomeBodyTarget-welcomeBodyStart)*fraction;
      renderWelcomeBody(welcomeBodyProgress);
      if(fraction<1){
    ...
    function setCamera(){
      ...
      var nextY;
      if(!camera){return;}
      nextX=clamp(state.x,-metrics.maxX,metrics.maxX);
      nextY=clamp(state.y,-metrics.maxY,metrics.maxY);
      if(nextX!==state.x){state.vx=0;}
      if(nextY!==state.y){state.vy=0;}
      state.x=nextX;
      state.y=nextY;
    ...
    function resizeRenderer(){
      ...
      if(!sourceImage.naturalWidth){
        sourceImage.addEventListener("load",queueResize,{once:true});
        return;
      }

      width=Math.max(activeHero.clientWidth,1);
      height=Math.max(activeHero.clientHeight,1);
      imageWidth=sourceImage.naturalWidth||1920;
      imageHeight=sourceImage.naturalHeight||1080;
      overscan=isSp()?1.16:1.1;
      coverScale=Math.max(width/imageWidth,height/imageHeight)*overscan;
      visibleWidth=width/coverScale;
      visibleHeight=height/coverScale;
    ...
    function resetHero(withFade){
      ...
      if(withFade&&webglStage){
        webglStage.classList.remove("is-resetting");
        void webglStage.offsetWidth;
        webglStage.classList.add("is-resetting");
        window.setTimeout(function(){
    ...
    function captureCurrentFrame(){
      var ratio;
      if(!renderer||!webglStage){return null;}
      ...
      snapshotCanvas.height=Math.max(1,Math.round(metrics.height*ratio));
    ...
    function getWelcomeCompleteTop(){
      ...
      var stageHeight=welcomeStage
        ?Math.max(welcomeStage.scrollHeight,welcomeStage.offsetHeight,1)
        :viewport*.7;
      var stageTop=welcomeStageTop;
      if(stageTop===null){
        var imageCenter=snapshot
          ?snapshot.getBoundingClientRect().top+metrics.height*.5
          :viewport*.5;
      ...
      var travel=Math.max(stageTop+stageHeight-viewport*.5,viewport*.5);
    ...
    function syncSnapshotLayer(circleDone){
      if(!snapshot||released||!welcomeWrap){return;}
      if(circleDone&&snapshot.parentNode!==welcomeWrap){
        welcomeWrap.insertBefore(snapshot,welcomeWrap.firstChild);
      }else if(!circleDone&&snapshot.parentNode!==document.body){
        document.body.appendChild(snapshot);
      }
    }
    ...
    function updateScrollEffects(){
      ...
      var heroTop=documentTop(hero);
      var welcomeTop=welcomeWrap?documentTop(welcomeWrap):heroTop+window.innerHeight;
      var travelMetrics;
      var circleProgress;
      var startRadius;
      var targetRadius;
      ...
      if(!welcomeActive||!snapshot||!welcomeWrap){return;}

      circleProgress=clamp(
        (scrollY-heroTop)/Math.max(welcomeTop-heroTop,1),
        0,1
      );
      ...
      if(welcomeStage&&welcomeStageTop!==null){
        ...
        if(wasFixed&&!keepFixed){
          debugWelcomeSnapshot('文字固定解除・直前');
          /* 固定中の実際の表示位置を、そのまま通常配置へ渡す。 */
          welcomeStageExitTop=welcomeStage.getBoundingClientRect().top-
        ...
        if(wasFixed&&!keepFixed){
          debugWelcomeSnapshot('文字固定解除・直後');
          window.requestAnimationFrame(function(){
            debugWelcomeSnapshot('文字固定解除・次フレーム');
          });
          window.setTimeout(function(){
            debugWelcomeSnapshot('文字固定解除・0.5秒後');
          },500);
        }
        if(keepFixed&&!debugWelcomeCircleLogged){
          debugWelcomeCircleLogged=true;
          debugWelcomeSnapshot('丸完成・文字固定開始');
        }
      }
      if(welcomeDotPending&&circleProgress>=.999&&
        scrollY>=welcomeTop-1){
        welcomeDotPending=false;
        welcomeDotTimer=window.setTimeout(function(){
          welcomeDotTimer=0;
          if(welcomeActive&&snapshot&&
            window.pageYOffset>=documentTop(welcomeWrap)-2){
            scrollToWelcomeComplete();
          }
        },650);
      }
      ...
      updateWelcomeBody(circleProgress>=.999,clamp(
        (scrollY-welcomeTop)/Math.max(travelMetrics.travel-1,1),0,1
      ));
      ...
      if(scrollY-welcomeTop>=travelMetrics.travel){
        if(!welcomeBodyCompleted){
          ...
          welcomeBodyCompleted=true;
          renderWelcomeBody(1);
          debugWelcomeSnapshot('終点で本文表示完了');
        }
        releaseSnapshot();
      }else if(released){
        refixSnapshot();
    ...
    function onPointerMove(event){
      ...
      state.y=dragStart.cameraY+dy/Math.max(metrics.scale,.001);
      setCamera();

      now=performance.now();
      elapsed=Math.max(now-dragStart.lastTime,8);
      state.vx=-(event.clientX-dragStart.lastX)/Math.max(metrics.scale,.001)*(16/elapsed);
      state.vy=(event.clientY-dragStart.lastY)/Math.max(metrics.scale,.001)*(16/elapsed);
    ...
    function buildThree(THREE){
      ...
      if(sourceImage.complete&&sourceImage.naturalWidth){complete();}
        ...
        sourceImage.addEventListener("load",complete,{once:true});
        sourceImage.addEventListener("error",createFallback,{once:true});
      }
    }
    ...
    window.hexHero={
      ...
      scrollToWelcomeComplete:function(){
        window.clearTimeout(welcomeDotTimer);
        welcomeDotTimer=0;
        if(!welcomeActive){
          /* 丸が完成するまでを先に通過し、見出しの後で本文へ進む。 */
          welcomeDotPending=true;
          beginWelcome();
        }else if(window.pageYOffset<documentTop(welcomeWrap)-1){
          welcomeDotPending=true;
          var circleEnd=Math.max(documentTop(welcomeWrap),0);
          if(window.hexMotion&&typeof window.hexMotion.scrollTo==="function"){
            window.hexMotion.scrollTo(circleEnd,{duration:1.05,force:true});
          }else{
            window.scrollTo({top:circleEnd,behavior:reduced?"auto":"smooth"});
          }
        }else{
          scrollToWelcomeComplete();
        }
...
hexReady(function(){
  ...
  function createOpeningReplayButton(){
    ...
    function requestReplayUpdate(){
      if(updateRequested){return;}
      updateRequested=true;
      window.requestAnimationFrame(updateReplayVisibility);
    }
  ...
  function collectOpeningIntro(){
    var source=document.querySelector(".hex-opening-intro-source");
    var pcImage;
    var spImage;
    var main;
    var sub;
    ...
    pcImage=source.querySelector(".hex-opening-intro-pc img");
    spImage=source.querySelector(".hex-opening-intro-sp img");
    main=source.querySelector(".hex-opening-intro-main");
    sub=source.querySelector(".hex-opening-intro-sub");

    if(!pcImage&&!spImage){
      return null;
    }

    pcImage=pcImage||spImage;
    spImage=spImage||pcImage;

    data={
      pcSrc:pcImage.currentSrc||
        pcImage.getAttribute("src")||
        pcImage.getAttribute("data-src")||"",
      spSrc:spImage.currentSrc||
        spImage.getAttribute("src")||
        spImage.getAttribute("data-src")||"",
      alt:pcImage.getAttribute("alt")||"",
      pcPosition:pcImage.getAttribute("data-position")||"center",
      spPosition:spImage.getAttribute("data-position")||"center",
  ...
  function createOpeningIntro(opening,data){
    ...
    stage.className="hex-opening-intro-stage";
    picture.className="hex-opening-intro-picture";
    source.media="(max-width:768px)";
    source.srcset=data.spSrc;

    image.className="hex-opening-intro-photo";
    image.src=data.pcSrc;
    image.alt=data.alt;
    image.decoding="async";
    image.loading="eager";
    image.fetchPriority="high";
    image.draggable=false;
    image.style.setProperty("--hex-intro-pc-position",data.pcPosition);
    image.style.setProperty("--hex-intro-sp-position",data.spPosition);

    picture.appendChild(source);
    picture.appendChild(image);
    stage.appendChild(picture);

    catchBox.className="hex-opening-intro-catch";
    ...
    if(data.sub){
      sub.className="hex-opening-intro-sub";
      sub.innerHTML=data.sub;
      catchBox.appendChild(sub);
    }

    if(data.main||data.sub){
      stage.appendChild(catchBox);
    }
  ...
  function createOpeningElement(){
    ...
    curtain.innerHTML=
      '<span class="hex-opening-curtain-track">'+
        '<i></i><i></i><i></i><i></i>'+
      '</span>';
  ...
  function createOpeningProgressNav(opening,sceneCount,heroDotIndex){
  ...
  function startOpeningScroller(
    ...
    if(messageIndex<0){
      messageIndex=Math.max(0,brandIndex-1);
    }
    scenes.push(null);
    heroIndex=scenes.length-1;

    /* ヒーローはドットに含めず、7番目をロゴ完成位置にする。 */
    nav=createOpeningProgressNav(opening,heroIndex);
    cue=createOpeningScrollCue(opening);

    function setSceneStyle(scene,x,y,isVisible){
      if(!scene){
        return;
      }
    ...
    function createHeroPreview(){
      ...
      activeHero=window.hexHero&&
        typeof window.hexHero.getActive==="function"
          ?window.hexHero.getActive()
          :null;
      heroSticky=document.querySelector(".hex-hero-sticky");
      ...
      heroPreview.className=
        "hex-hero-image-handoff hex-opening-hero-preview";
      heroPreview.setAttribute("aria-hidden","true");

      clone=activeHero.cloneNode(true);
      clone.classList.add("hex-hero-handoff-clone");
      clone.removeAttribute("id");
      removableItems=clone.querySelectorAll(
        ".hex-hero-catch,.hex-scroll-indicator,.hex-opening-replay"
      );
      removableItems.forEach(function(item){
        item.remove();
      });
      clone.style.marginLeft=heroSticky
    ...
    function updateDots(){
      ...
      if(position>=brandIndex-.001){
        current=brandIndex;
      }else if(position>=messageIndex){
    ...
    function render(){
      var storyProgress=phase(position,messageIndex,brandIndex);
      var verticalProgress=phase(storyProgress,0,.22);
      var revealProgress=phase(position,brandIndex,heroIndex);
      ...
      scenes.forEach(function(scene,index){
        ...
        if(position>=messageIndex&&index===messageIndex){
          x=0;
          y=-100*verticalProgress;
          isVisible=verticalProgress<1;
        }else if(position>=messageIndex&&index===brandIndex){
          x=0;
          y=100*(1-verticalProgress);
          isVisible=true;
        }else{
          x=(index-position)*100;
          isVisible=Math.abs(x)<125;
        }
      ...
      if(revealProgress>0){
        ...
        if(heroPreview){
          heroPreview.classList.remove("is-active");
        }
    ...
    function positionToLocal(target){
      if(target<=messageIndex){
        return target*sceneDistance;
      }
      if(target<=brandIndex){
        return preStoryDistance+
          (target-messageIndex)*storyDistance;
      }
      return preStoryDistance+storyDistance+
        (target-brandIndex)*circleDistance;
    }

    function localToPosition(local){
      if(local<=preStoryDistance){
        return local/sceneDistance;
      }
      if(local<=preStoryDistance+storyDistance){
        return messageIndex+
          (local-preStoryDistance)/storyDistance;
      }
      return brandIndex+
        (local-preStoryDistance-storyDistance)/circleDistance;
    }
    ...
    function measure(){
      ...
      preStoryDistance=Math.max(messageIndex*sceneDistance,1);
      storyDistance=sceneDistance*4;
      circleDistance=sceneDistance;

      hostTop=host.getBoundingClientRect().top+window.pageYOffset;
      scrollDistance=Math.max(
        preStoryDistance+storyDistance+circleDistance,
        1
      );
    ...
    function updateFromScroll(){
      ...
      position=localToPosition(local);
      render();
    }
  ...
  /*
   * 2026 rebuild: fixed-position scene replacement with a four-band curtain.
   * Images do not travel; only the curtain and the incoming copy/logo move.
   */
  function startOpeningCurtainSequence(
    opening,
    introStage,
    slideStage,
    messageStage,
    ...
    var timers=[];
    var curtain=opening.querySelector(".hex-opening-curtain");
    var track=curtain&&curtain.querySelector(".hex-opening-curtain-track");
    var bands=track
    ...
    function wait(delay){
      return new Promise(function(resolve){later(resolve,delay);});
    }
    ...
    function cancel(){
      ...
      if(track&&track.getAnimations){
        track.getAnimations().forEach(function(animation){animation.cancel();});
      }
    ...
    function activate(nextIndex){
      scenes.forEach(function(scene,sceneIndex){
        var active=sceneIndex===nextIndex;
    ...
    function collectPreloadUrls(){
      ...
      if(hero){
        Array.prototype.forEach.call(hero.querySelectorAll("img"),function(image){
          var source=image.currentSrc||image.src;
          if(source){urls.push(source);}
        });
      }
    ...
    function showLoader(){
      ...
      function finish(resolve){
        ...
        var remaining=Math.max(700-(Date.now()-started),0);
        later(function(){
